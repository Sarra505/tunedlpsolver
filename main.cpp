...
int main(int argc, const char * argv[]) {
    ...
    std::istringstream matrixEntryStream(line);
    int numRules;
    matrixEntryStream >> numRules;
    ...
    // now fill the nonzeroes
    for (size_t row = 0; row < m; row++)
    {
        int numEntries;
        matrixEntryStream >> numEntries;

        for (size_t i = 0; i < numEntries; i++)
        {
            int columnNumber;
            float coefficient;
            matrixEntryStream >> columnNumber >> coefficient;
            A[row * (m+n) + columnNumber] = coefficient;
        }
    }
